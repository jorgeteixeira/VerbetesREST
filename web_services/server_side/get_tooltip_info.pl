...
## How to use it:
##
##	cgi-bin/VerbetesREST/get_tooltip_info.pl?name=Pedro Passos Coelho&beginDate=2012-04-10&endDate=2012-04-18
##
use strict;
use warnings;
use CGI qw/:standard/;
...
if ($name ne "") {
	...
	# memcached
	my $key = "Verbetes::MVDI::" . $name . "::" . $begin_date . "::" . $end_date;
	...
	if (!defined($memcached_content)) {
		$tooltip_info = $server->GetTooltipInfo($name, $begin_date, $end_date);
		$memcached_handler->SetMemcached($key, $tooltip_info, 3600*24);
		warn("[$key] empty\n");	
	} else {
		$tooltip_info = $memcached_content;
		warn("[$key] cached\n");
	}	
	
	if ( $tooltip_info eq "ERROR" ) {	$error = $json_obj->PrepareError(1,"Invalid format of the input paramenter(s)"); } 
	if ( $tooltip_info eq "ERROR2" ) {	$error = $json_obj->PrepareError(2,"Maximum time interval is one month"); }	
	else { $output = $tooltip_info; }
}
