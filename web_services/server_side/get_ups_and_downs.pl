...
my $num_results = $cgi->param("numResults") || 10;
...
#$info = $server->GetUpsAndDowns($begin_date, $end_date);
# memcached
my $key = "VerbetesREST::GetUpsAndDowns::" . $begin_date . "::" . $end_date . "::" . $num_results;
...
my $memcached_content = $memcached_handler->GetMemcached($key);
	...
	$info = $server->GetUpsAndDowns($begin_date, $end_date, $num_results);		
